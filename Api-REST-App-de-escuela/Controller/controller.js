...
var controller = {
    ...
    agregar_Alumno: function (req, res) {
        ...
        modelo.save((err, Guardar_Info)=>{
            ...
            if(!Guardar_Info) return res.status(404).send({message: 'No se ha podido guardar'})
    ...
    obtenerAlumno: function (req, res){
        ...
        Modelo.findById(IdAlumno, (err, Alumno)=>{
            ...
            if(!Alumno) return res.status(404).send({message:"Infomacion no existe"})
    ...
    obtenerTodosLosAlumnos: function(req, res){
        ...
        Modelo.find({}).sort('-edad').exec((err, alumnos)=>{
            if(err) return res.status(500).send({message: "error"})
            if(!alumnos) return res.status(404).send({message: "No hay ALumnos Registrados"})
    ...
    actualizar_Reg_Alumos: function(req, res){
        ...
        Modelo.findByIdAndUpdate(IdAlumno, update, {new: true}, (err, AlumnoActualzado)=>{
            if(err) return res.status(500).send({message:'Error al devolver actualizar'})
            if(!AlumnoActualzado) res.status(404).send({message: 'informacion no existe'})
    ...
    BorrarAlumno: function(req, res){
        ...
        Modelo.findByIdAndDelete(IdAlumno, (err, AlumnoEliminado)=>{
            if(err) return res.status(500).send({message: "Error Al eliminar alumno"})
            if(!AlumnoEliminado) return res.status(404).send({message: "Informacion no existe"})
    ...
    subirImagen: function(req, res){
        ...
        if(req.files){
            ...
            if(fileExt == 'png' || fileExt == 'jpg' || fileExt == 'jpeg' || fileExt == 'gif'){
                Modelo.findByIdAndUpdate(ImagenId, {image: fileName}, {new: true}, (err, ImageUpdated) => {
                    if(err) return res.status(500).send({message: "error al enviar"})
                    if(!ImageUpdated) return res.status(404).send({message: "Imagen no existente o no subida"})      
